...
\begin{equation}
  \phi_c({\bf x}_c) = \omega_c \sum_{i = 1}^L \max_{(a, b) \in c} \cdot |y_{ai} - y_{bi}| 
\label{eq:hod_ip}
\end{equation}
...
\begin{align}
    ...
    \sum_{(a, b) \in {\mathcal N}} \sum_{i} \frac{w_{ab}}{2} \cdot |y_{ai} - y_{bi}| \nonumber\\
    & + \quad \sum_{c \, \in {\mathcal C}} \frac{w_c}{2} \sum_{i = 1}^L \max_{(a, b) \in c} \cdot |y_{ai} - y_{bi}| \nonumber\\
    \text{s.t} \quad &{\bf y} \in \Delta.
\label{eq:hop-lp}
\end{align}
...
this LP same as (HOD-LP) of equation (\ref{eq:hop-lp}), thereby enabling us to recover the submodular extension
for higher-order diversity model. The following proposition gives us the submodular
extension.
{\proposition The Lovasz extension of the following submodular extension for
  our higher-order diversity model
  \begin{align}
      F_{HOD}(A) &= F_{Potts}(A) + \nonumber \\
        &\sum_{c \in {\mathcal C}} \frac{w_c}{2} \sum_{i = 1}^L \max_{(a, b) \in c} [|A \cap \{v_{ai}, v_{bi}\}| = 1]
\end{align}
is the objective function $E({\bf y})$ of the LP relaxation (HOD-LP).
\label{proposition:hod_extension}}
\newline
