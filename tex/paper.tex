...
\myparagraph{Data} Next, we use the higher-order Potts model for stereo
matching on the Middlebury dataset. For this
purpose, we take a dense CRF model with Potts compatibility term and Gaussian pairwise
potentials, and augment it with higher-order Potts term.
...
\myparagraph{Results} Figure \ref{fig:stereo_hop} shows some example solutions obtained by picking the
label with maximum marginal probability for each variable.
