...
Equation (\ref{eq:hod_ip}) suggests that our diversity model can be interpreted as sum of $P^n$ Ising models over all labels. The $Pn$ Ising model corresponding to label $i$ is $\max_{(a, b) \in c} \cdot |y_{ai} - y_{bi}|$ and it imposes a penalty of 0 if all variables in clique $c$ take the same label $i$, else the penalty is 1.
...
this LP same as (HOD-LP) of equation (\ref{eq:hop-lp}), thereby enabling us to recover the submodular extension for higher-order diversity model. The following lemma will be of use to us in indentifying the extension.
...
\newline
We note that the component of the submodular extension corresponding to cliques is of the form of maximum of cut function (that are submodular) over all pairs in a clique. In general, maximum of submodular functions is not submodular. However, in our particular case, the maximum turns out to be submodular as shown in the proof below. Our proof makes use of the following property:
